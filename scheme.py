...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        if not scheme_procedurep(operator): #checks if it's really an operator
            raise SchemeError ("'operator' is not a valid operator")
        #need to deal with nested exprs
        operands = rest.map(lambda arg: scheme_eval(arg, env)) #simplify everything
...
class Frame:
    ...
    def make_child_frame(self, formals, vals):
        ...
        if len(formals) != len(vals):
            raise SchemeError('Value do not match frames, u suck')
        while formals != nil:
...
class LambdaProcedure(UserDefinedProcedure):
    ...
    def make_call_frame(self, args, env):
        ...
        vals = args
        return Frame.make_child_frame(self.env, formals, vals)
        # END PROBLEM 12
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        env.define(target, rest) #in parenthesis when no supposed to be
        return target
        # END PROBLEM 6
        ...
        l_expr = LambdaProcedure(formals, body, env)
        env.define(target.first, l_expr)
        return target.first
        # END PROBLEM 10
    else:
...
def do_and_form(expressions, env):
    ...
    if expressions == nil:
        return True
    while expressions.second != nil:
        if scheme_falsep(scheme_eval(expressions.first, env)): #different false values in scheme
            return False
        else:
            expressions = expressions.second
    return scheme_eval(expressions.first, env)
